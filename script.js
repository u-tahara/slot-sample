...
export class PachinkoGame {
  ...
  buildReelSequence(outcome) {
    ...
    const pickSymbol = () => {
      ...
      return symbols[index] ?? symbols[0];
    };
    ...
    const triggerReach = Math.random() < reachChance;

    if (triggerReach) {
      second = first;
      if (third === first) {
        third = symbols.find((symbol) => symbol !== first) ?? third;
      }
    } else if (first === second && second === third) {
      third = symbols.find((symbol) => symbol !== second) ?? third;
    }
