...
function createTestDocument({ initialCredits = 120 } = {}) {
  ...
  const hitRateDisplay = new MockElement('hit-rate-display', doc);
  const rushRateDisplay = new MockElement('rush-rate-display', doc);
  ...
  doc.registerElement('hit-rate-display', hitRateDisplay);
  doc.registerElement('rush-rate-display', rushRateDisplay);
...
test('probability displays reflect configured hit and rush rates', () => {
  const { doc, elements } = createTestDocument();
  const game = mountPachinko(
    doc,
    createTestConfig({ hitRate: 0.137, rushRate: 1 / 3 })
  );

  assert.equal(elements.hitRateDisplay.textContent, '13.7%');
  assert.equal(elements.rushRateDisplay.textContent, '33.3%');

  game.config.hitRate = 1 / 120;
  game.config.rushRate = 0.5;
  game.updateDisplays();

  assert.equal(elements.hitRateDisplay.textContent, '1/120');
  assert.equal(elements.rushRateDisplay.textContent, '50%');
});
...
test('mountPachinko initialises displays, pockets, and startup log', () => {
  ...
  assert.equal(elements.hitRateDisplay.textContent, '1/2');
  assert.equal(elements.rushRateDisplay.textContent, '25%');
